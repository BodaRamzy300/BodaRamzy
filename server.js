...
client.login("NzQ0Mjc1NzUzMTgwMzMyMDg0.Xzg2zw.YVE0HuEsIZaTb8PpnAhKnyNX2Hc");
const queue = new Map();
...
client.on('message', async message => { 
    ...
    if (message == prefix + 'stock') {
        ...
        message.channel.send(new Discord.RichEmbed().setTitle(':fire: BH⌯EMPIRE :fire:') ///1
            .addField('**:desktop: SFA :desktop:**', `**${ahmed} Account(s)**`, true). 
            addField('**:desktop: NETFLIX :desktop:**', `**${hossam} Account(s)  **`, true).setColor('blue') 
            .setFooter(`$buy [sfa/net] [الحسابات لا يوجد عليها ضمان | لشراء حساب الرجاء كتابة الأمر التالي [الكمية`))
    }   
    if (message.content.startsWith(prefix + 'buy')) { 
        ...
        if (cmd == 'sfa') { 
          if (cools [message.author.id + message.guild.id] && cools [message.author.id + message.guild.id].status == "on")return message.reply("**لديك عملية شراء بل فعل.**"); 
            ...
            if (ahmed < 1) return message.channel.send("لا يوجد حسابات:x:")
            if (ahmed < args) return message.channel.send("لا يوجد حسابات كافية:x:") // 
...
لديك 3 دقائق قبل الالغاء.**`));
  ...
  if (message.content.startsWith(prefix + 'add')) {
        ...
        if (type == "sfa") {
            ...
            message.reply("**:white_check_mark:Successfully adedd this account.**");
